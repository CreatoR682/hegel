...
const LATER_VERIFICATION_PATTERN = /\b(test|tests|testing|test\.ts|\.test\.|spec\b|jest|vitest|mocha|cypress|playwright|npm run test|npx vitest|run tests?|test suite|coverage|build\b|npm run build|verify|verified|verification|validated?|passed|passing|all clean|green\b|linter|diagnostics|smoke test|no failures|security review|audit)\b/i;
/** Assistant messages that indicate automated tests or builds were run successfully. */
const ASSISTANT_VERIFICATION_PATTERN = /\b(\d+\s+tests?\s+passed|tests?\s+passed|vitest(?:\s+run)?|npm run test|test files?\s+(?:\d+|passed)|all tests|exit code:\s*0|✓\s*\d+\s*test)\b/i;
function hasUserSideVerificationEvidence(state, transcript, pendingUserPrompt) {
    ...
    if (transcript &&
        recentUserTurns(transcript, 10).some((turn) => LATER_VERIFICATION_PATTERN.test(turn.text))) {
        return true;
    }
    return false;
}
function hasAssistantTestCompletionEvidence(transcript, lastAssistantText) {
    if (lastAssistantText && ASSISTANT_VERIFICATION_PATTERN.test(lastAssistantText)) {
        return true;
    }
    if (transcript &&
        recentAssistantTurns(transcript, 6).some((turn) => ASSISTANT_VERIFICATION_PATTERN.test(turn.text))) {
        return true;
    }
    return false;
}
/** True if this persisted concern is stale given later user or assistant verification activity. */
export function concernSupersededByLaterEvidence(concern, state, transcript, options) {
...
export function pruneSupersededConcernsFromState(state, transcript, options) {
    state.concerns = state.concerns.filter((c) => !concernSupersededByLaterEvidence(c, state, transcript, options));
}
/**
