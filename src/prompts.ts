...
const STRICTNESS_GUIDANCE: Record<StrictnessLevel, string> = {
  relaxed:
    "Only flag serious issues — vague one-word prompts, clearly dangerous operations, or missing context that will definitely cause problems. Let most prompts through.",
  balanced:
    "Flag prompts that lack specificity, miss acceptance criteria for significant tasks, or skip verification after major changes. Allow clear, purposeful prompts even if brief.",
  strict:
    "Scrutinize every prompt rigorously. Question assumptions, demand explicit acceptance criteria, flag any missing context, and challenge prompts that don't reference specific files or behaviors.",
};

export function buildPromptAnalysisPrompt(strictness: StrictnessLevel): string {
...
export function buildResponseAnalysisPrompt(strictness: StrictnessLevel): string {
