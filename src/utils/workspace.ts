...
export async function findAncestorHegelInstall(
  startDir: string,
...
const ORPHAN_DIR_NAMES = ["hegel-mcp", "init"] as const;
const ORPHAN_MARKER_PATHS = ["hegel.config.json", join(".cursor", "hooks.json")] as const;
...
export async function pruneOrphanInstalls(
  projectPath: string,
  deps: Pick<SetupDeps, "access" | "rm" | "log">
): Promise<PruneResult> {
