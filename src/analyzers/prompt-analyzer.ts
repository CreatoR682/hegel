...
const LAZY_PATTERNS: Array<{ pattern: RegExp; label: string }> = [
...
const MISSING_CONTEXT_SIGNALS = [
  { test: (p: string) => p.length < 15 && !p.includes("?"), message: "Very short prompt without a question — the AI will guess your intent" },
  { test: (p: string) => /\b(it|this|that|these|those)\b/i.test(p) && p.split(/\s+/).length < 8, message: "Pronoun-heavy short prompt — 'it'/'this' may be ambiguous after context compaction" },
];
...
export function analyzePrompt(
...
): { concerns: Concern[]; shouldBlock: boolean } {
  ...
  for (const { test, message } of MISSING_CONTEXT_SIGNALS) {
    if (test(prompt)) {
      if (isFollowUp && state.compactionCount < 3) continue;
  ...
  const capWc = (count: number) => Math.min(count, 100);
  ...
  if (ctx && ctx.userTurnCount >= 3) {
    ...
    if (lastUserTurns.length >= 3) {
      const similarities = countSimilarPairs(
        lastUserTurns.map((t) => t.text.toLowerCase())
      );
      if (similarities >= 2) {
        concerns.push({
          severity: "info",
    ...
    if (lastUser && lastAssistant) {
      ...
      if (overlapRatio < 0.1 && currentWords.size > 8) {
        ...
        if (firstUserTurn && firstUserTurn !== ctx.turns.find(t => t.text === lastUser)) {
          ...
          if (planRatio >= 0.15) relatedToSessionPlan = true;
        }

        const releaseThreadFollowUp =
          RELEASE_OR_INSTALL_THREAD.test(prompt) ||
  ...
  const shouldBlock = concerns.some(
 ...
 */
function countSimilarPairs(texts: string[]): number {
  let count = 0;
...
function recentReleaseThreadContext(
  state: SessionState,
  ...
  const fromState = recentPrompts(state, 5)
    .map((r) => r.prompt)
    .join("\n");
  if (RELEASE_OR_INSTALL_THREAD.test(fromState)) return true;
