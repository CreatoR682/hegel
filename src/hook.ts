...
import { loadConfig, type HegelConfig } from "./config.js";
import {
  loadState,
  saveState,
  addPrompt,
...
import {
  analyzeResponse,
  buildResponseRecord,
} from "./analyzers/response-analyzer.js";
import {
  analyzeSession,
  pruneSupersededConcernsFromState,
} from "./analyzers/session-analyzer.js";
 ...
 */
function normalizedWorkspaceRoots(input: HookBaseInput): string[] {
 ...
 */
async function isSelfProject(input: HookBaseInput): Promise<boolean> {
...
async function shouldBypass(input: HookBaseInput, config: HegelConfig): Promise<boolean> {
  return isSelfProject(input);
}

async function log(level: string, event: string, message: string): Promise<void> {
  try {
...
function shortConversationId(conversationId: string | undefined): string {
...
function writeOutput(obj: Record<string, unknown>): void {
 ...
 */
async function maybeRegenerateHooks(
...
async function handleBeforeSubmitPrompt(
  ...
  if (input.composer_mode) state.composerMode = input.composer_mode;
...
async function handleStop(input: StopInput, output: OutputWriter): Promise<void> {
  const state = await loadState(input.conversation_id);
...
async function handlePreCompact(input: PreCompactInput, output: OutputWriter): Promise<void> {
  const workspaceRoots = normalizedWorkspaceRoots(input);
  const config = await loadConfig(workspaceRoots[0] || process.cwd());
  const bypass = await shouldBypass(input, config);
  const state = await loadState(input.conversation_id);
...
async function handleSessionStart(
  ...
  output({
    additional_context:
      `[Hegel dialectical companion is ${mode} for this session. It will monitor prompt quality, response patterns, and session health.]`,
  });
}
...
export async function processHookInput(
  input: HookBaseInput,
  output: OutputWriter = writeOutput
): Promise<void> {
  ...
  switch (event) {
    case "beforeSubmitPrompt":
      await handleBeforeSubmitPrompt(input as BeforeSubmitPromptInput, output);
      break;
    case "afterAgentResponse":
      await handleAfterAgentResponse(input as AfterAgentResponseInput, output);
      break;
    case "afterFileEdit":
      await handleAfterFileEdit(input as AfterFileEditInput, output);
      break;
    case "stop":
      await handleStop(input as StopInput, output);
      break;
    case "preCompact":
      await handlePreCompact(input as PreCompactInput, output);
      break;
    case "sessionStart":
      await handleSessionStart(input as SessionStartInput, output);
      break;
    default:
      output({});
  }
...
export async function processHookJson(
  raw: string,
  output: OutputWriter = writeOutput
): Promise<void> {
  ...
  try {
    input = JSON.parse(raw) as HookBaseInput;
  } catch {
    output({});
    return;
  }
...
export async function runHook(
  readInput: InputReader = readStdin,
  output: OutputWriter = writeOutput
): Promise<void> {
