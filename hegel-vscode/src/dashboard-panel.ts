...
import type { SessionState, Concern } from "./types";

export class DashboardPanel implements vscode.WebviewViewProvider {
  public static readonly viewType = "hegel.dashboard";
  ...
  private buildHtml(state: SessionState | null): string {
    if (!state) {
    ...
    const l2Model = config.get<string>("model") || "auto";
    const l2Enabled = config.get<boolean>("enableLlmAnalysis") !== false;
...
function normalizeSeverity(severity: string): Concern["severity"] {
